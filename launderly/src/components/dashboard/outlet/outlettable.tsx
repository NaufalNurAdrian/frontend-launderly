  ...
  useEffect(() => {
    const fetchOutlets = async () => {
      try {
        ...
        const calculatedTotalPages = Math.max(1, Math.ceil(activeOutletCount / limit));
      ...
      <div className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.isArray(outlets) && outlets.length > 0 ? (
            ...
            <div className="col-span-3 text-center py-10 text-gray-500">No active outlets available</div>
          )}
        </div>
      </div>
